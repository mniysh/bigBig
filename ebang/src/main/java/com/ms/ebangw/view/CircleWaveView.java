...
public class CircleWaveView extends View implements Runnable {
	...
	private void init() {
		...
		if (mWidth >= mHeight) {
			maxRadius = mHeight / 1.2F;
		}
		else {
			maxRadius = mWidth / 1.2F;
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		while (true) {
			int alpha = (int) (400.0F * (1.0F - radius / maxRadius));
			if (alpha <= 0) {
				break;
			}
